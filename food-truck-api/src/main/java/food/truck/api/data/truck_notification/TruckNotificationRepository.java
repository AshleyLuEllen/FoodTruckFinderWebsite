...
public interface TruckNotificationRepository extends JpaRepository<TruckNotification, Long> {
    ...
    @Query(value="(SELECT * FROM " + TruckNotification.TABLE_NAME + " tn WHERE tn.published = b'1' AND tn.truck_id IN (SELECT sub.truck_id FROM " + Subscription.TABLE_NAME + " sub WHERE sub.user_id = ?1)) UNION (SELECT * FROM " + TruckNotification.TABLE_NAME + " tn2 WHERE tn2.notification_id IN (SELECT un2.notification_id FROM " + UserNotification.TABLE_NAME + " un2 WHERE un2.user_id = ?1))", nativeQuery = true)
