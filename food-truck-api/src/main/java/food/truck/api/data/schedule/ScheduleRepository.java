...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
    ...
    @Query(value = "SELECT * FROM " + Schedule.TABLE_NAME + " WHERE ( 3959 * acos( cos( radians(?1) ) * cos( radians( truck_latitude ) ) " +
        "* cos( radians( truck_longitude ) - radians(?2) ) + sin( radians(?1) ) * sin(radians(truck_latitude)))) <= ?3 AND time_from < ?4", nativeQuery = true)
